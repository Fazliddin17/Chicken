...
public class Order {
    ...
    public void five(User user, CallbackQuery callbackQuery, String serverPath) {
        ...
        if (data.equals("success order")) {
            if (sum(baskets) < 10000) {
                bot.alertMessage(callbackQuery, lang.equals("uz") ? "Buyurtmaning eng kam miqdori 10 000 so'mni tashkil etadi" : (lang.equals("en") ? "The minimum amount of the order is 10,000 soums" : "Минимальная сумма заказа 10 000 сум."));
                return;
            }
            ...
            if (lang.equals("uz")) {
                ...
                a = """
                        ...
                        To'lov turi:
                        """.formatted(0000.0, sum(baskets), user.getCurrentLocation(), list1.get(0).getFullAddress());
            }
...
//            String captionBasket = ;
            ...
            if (user.getLang().equals("uz")) {
                h = """
                        ...
                        Buyurtmangizni Masofaga qarab uzogi 30  daqiqa ichida yetkazib beramiz.
                        """.formatted(user.getBaskets().get(user.getBaskets().size() - 1).getId());
            }
            if (user.getLang().equals("ru")) {
                h = """
                        Ваш заказ: %d находится в обработке!
                        Мы доставим ваш заказ в течение 30 минут в зависимости от расстояния.
                        """.formatted(user.getBaskets().get(user.getBaskets().size() - 1).getId());
            }
            if (user.getLang().equals("en")) {
                h = """
                        Your order: %d is being processed!
                        We deliver the order within 30 minutes depending on the distance.
                        """.formatted(user.getBaskets().get(user.getBaskets().size() - 1).getId());
            }
            bot.deleteMessage(user.getChatId(), callbackQuery.getMessage().getMessageId());
